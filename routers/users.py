...
async def get_current_active_user(current_user: schemas.UserInfo = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def read_users_me(current_user: schemas.UserInfo = Depends(get_current_active_user), db: Session = Depends(get_db)):
    return schemas.UserInfo(**current_user.dict())
...
@router.get("/users/", response_model=List[schemas.UserList], deprecated=True)
def read_users(token: str = Depends(oauth2_scheme),skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/users/{user_id}", response_model=schemas.UserList)
